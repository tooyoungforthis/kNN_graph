...
def generate_clustered_graph(N: int, n_clusrers: int) -> nx.Graph:
    """Генерация графа c N вершинами c разбиением на n_clusters кластеров
...
def clustering_graph(Graph: nx.Graph, n_clusters) -> Dict[int, int]:
    """Кластеризаия графа на основе расстояния до ближайщих соседей"""
...
def _get_shortest_path_matrix(Graph: nx.Graph) -> np.ndarray:
    """Считаем кратчайшее расстояние между всеми парами вершин"""
...
def _get_closest_neighbours(Graph: nx.Graph, shortest_path_matrix: np.ndarray) -> Dict[int, Set[int]]:
    """Находим соседей какждой вершины
...
def _compute_cluster_(Graph: nx.Graph, n_clusters: int, nodes_neighbours: Dict[int, Set[int]]) -> Dict[int, int]:
    """Находим кластеры для большинства вершин"""
...
def _compute_cluster_for_particular_nodes(Graph: nx.Graph, node_and_its_cluster: Dict[int, int], need_to_cluster_nodes: List[int], nodes_neighbours: Dict[int, Set[int]]) -> Dict[int, int]:
    """Найдем кластеры точек, которые не смогли найти на предыдущем этапе
    Конечный кластер будет определяться, как самый часто соседствующий кластер(мода)
...
def draw_graph(Graph: nx.Graph, clusters: Dict[int, int]) -> None:
    """Выводит изначальный граф и кластеризированный граф"""
...
########################################
# НУЖНО ДОРАБОТАТЬ _compute_clusters!!!#
########################################
